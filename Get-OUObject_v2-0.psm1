...
.EXAMPLE
	Get-OUObjects -Category $Category -Objects $InputObject -Base $Base -Scope $Scope -OperatingSystem $OperatingSystem
            ...
            if ($cont -eq $true) {
                ...
                switch ($Category) {
                    "computer" {
                        ...
                        $ds.Filter = "(&(objectClass=Computer)(Name=ME751*INF*))"
                        $outs = $ds.FindAll()
                        foreach ($in in $outs) {
                            #$out | Out-GridView
                            $item = $in.Properties
                            $out += [PSCustomObject]@{
                                ...
                                Title                  = "N/A"
                                SID                    = $item.objectsid.Item(0)
                                OperatingSystem        = $item.operatingsystem.Item(0)
                                OperatingSystemVersion = $item.operatingsystemversion.Item(0)
                                DistinguishedName      = $item.distinguishedname.Item(0)
                                Created                = $item.whencreated.Item(0)
                                Modified               = $item.whenchanged.Item(0)
                                LastLogonDate          = (Get-Date $item.lastlogontimestamp.Item(0))
                                PasswordLastSet        = (Get-Date $item.pwdlastset.Item(0))
                            }
                        }
                    }
                    "user" {
